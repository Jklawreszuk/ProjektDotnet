    ...
    public class ArticleController : Controller
    {
        ...
        public IActionResult Index(int id)
        {
            Recipe = _context.Recipe
                .Include(r => r.User).Include(r => r.Images).Include(r => r.Ingredients).Where(n => n.Id == id).First();
            return View(Recipe);
        }
        ...
        public IActionResult AddToFavourites([FromForm] int RowId)
        {
            ...
            var result = _context.Favourites.Find(_userManager.FindByNameAsync(User.Identity.Name).Result.Id, RowId);
        ...
        public IActionResult Like([FromForm] int RowId)
        {
            ...
            if (result == null)
            {
                ...
                Recipe = _context.Recipe
                .Include(r => r.User).Include(r => r.Images).Include(r => r.Ingredients).Where(n => n.Id == RowId).First();
                Recipe.LikeCount += 1;
        ...
        [HttpPost, Authorize]
        public IActionResult Dislike([FromForm] int RowId)
        {
            var ratings = new Ratings()
            {
                RecipeId = RowId,
